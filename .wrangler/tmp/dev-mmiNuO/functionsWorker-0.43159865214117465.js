...
// .wrangler/tmp/bundle-AujGGg/checked-fetch.js
...
// .wrangler/tmp/pages-CVLbWR/functionsWorker-0.43159865214117465.mjs
...
var init_checked_fetch = __esm({
  "../.wrangler/tmp/bundle-Y7cWqy/checked-fetch.js"() {
    "use strict";
...
var init_email_notifications = __esm({
  "api/auth/shared/email-notifications.ts"() {
    "use strict";
    init_functionsRoutes_0_791931610879439();
    init_checked_fetch();
    ...
    const html = `
      ...
      <body>
        <script>
          ...
          window.location.replace('${env.SITE_URL || "http://localhost:3000"}/');
        <\/script>
        <p>Redirecting...</p>
      </body>
      </html>
...
async function signJwt2(payload, secret, expiresIn = 7 * 24 * 60 * 60) {
  const now = Math.floor(Date.now() / 1e3);
  const jwtPayload = {
    ...payload,
    iat: now,
...
async function signJwt5(payload, secret, expiresIn = 7 * 24 * 60 * 60) {
  const now = Math.floor(Date.now() / 1e3);
  const jwtPayload = {
    ...payload,
    iat: now,
...
var routes;
var init_functionsRoutes_0_791931610879439 = __esm({
  "../.wrangler/tmp/pages-CVLbWR/functionsRoutes-0.791931610879439.mjs"() {
    "use strict";
    init_callback();
    init_callback2();
    init_callback3();
...
// .wrangler/tmp/bundle-AujGGg/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-AujGGg/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.43159865214117465.js.map
