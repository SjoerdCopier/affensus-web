...
// .wrangler/tmp/bundle-KfSGsg/checked-fetch.js
...
// .wrangler/tmp/pages-z5RCgW/functionsWorker-0.9096654059469159.mjs
...
var init_checked_fetch = __esm({
  "../.wrangler/tmp/bundle-ILKrK0/checked-fetch.js"() {
    "use strict";
...
var init_email_notifications = __esm({
  "api/auth/shared/email-notifications.ts"() {
    "use strict";
    init_functionsRoutes_0_19345381520921912();
    init_checked_fetch();
...
async function onRequestGet3(context) {
  try {
    ...
    console.log("\u{1F511} Creating JWT token with payload:", jwtPayload);
    ...
    console.log("\u{1F3AB} JWT Token created (first 20 chars):", jwtToken.substring(0, 20));
    console.log("\u{1F3AB} JWT Token length:", jwtToken.length);
    ...
    const isProduction = env.SITE_URL?.startsWith("https://") || false;
    const secureFlag = isProduction ? "Secure; " : "";
    console.log("\u{1F36A} Setting up cookies...");
    console.log("\u{1F512} Is Production:", isProduction);
    console.log("\u{1F512} Secure flag:", secureFlag);
    const headers = new Headers({
      "Content-Type": "text/html"
    });
    const cookieValue = `auth-token=${jwtToken}; HttpOnly; ${secureFlag}SameSite=Lax; Max-Age=${7 * 24 * 60 * 60}; Path=/`;
    console.log("\u{1F36A} Auth cookie being set:", cookieValue);
    headers.append("Set-Cookie", cookieValue);
    headers.append("Set-Cookie", `oauth_state=; HttpOnly; ${secureFlag}SameSite=Lax; Max-Age=0; Path=/`);
    console.log("\u{1F36A} All headers set, returning response");
    return new Response(html, {
      status: 200,
      headers
    });
...
async function onRequestGet16(context) {
  try {
    const { request, env } = context;
    console.log("\u{1F50D} /api/user - Starting authentication check");
    const cookieHeader = request.headers.get("Cookie");
    console.log("\u{1F36A} Cookie header:", cookieHeader);
    let token = null;
    if (cookieHeader) {
      const cookies = cookieHeader.split("; ");
      console.log("\u{1F36A} Parsed cookies:", cookies);
      const authCookie = cookies.find((c) => c.startsWith("auth-token="));
      console.log("\u{1F3AB} Auth cookie found:", !!authCookie);
      if (authCookie) {
        token = authCookie.split("=")[1];
        console.log("\u{1F511} Token extracted (first 20 chars):", token?.substring(0, 20));
      }
    }
    if (!token) {
    ...
    const jwtSecret = env.JWT_SECRET;
    console.log("\u{1F510} JWT Secret available:", !!jwtSecret);
    if (!jwtSecret) {
    ...
    try {
      console.log("\u{1F50D} Attempting to verify JWT token...");
      const payload = await verifyJwt(token, jwtSecret);
      console.log("\u{1F3AF} JWT verification result:", !!payload);
      console.log("\u{1F4C4} Payload:", payload);
      if (!payload) {
...
var routes;
var init_functionsRoutes_0_19345381520921912 = __esm({
  "../.wrangler/tmp/pages-z5RCgW/functionsRoutes-0.19345381520921912.mjs"() {
    "use strict";
    init_callback();
    init_callback2();
    init_callback3();
...
// .wrangler/tmp/bundle-KfSGsg/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-KfSGsg/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9096654059469159.js.map
