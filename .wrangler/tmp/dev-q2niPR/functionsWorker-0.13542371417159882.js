...
// .wrangler/tmp/bundle-6TbaVW/checked-fetch.js
...
// .wrangler/tmp/pages-OHMer6/functionsWorker-0.13542371417159882.mjs
...
var init_checked_fetch = __esm({
  "../.wrangler/tmp/bundle-Ei7lCv/checked-fetch.js"() {
    "use strict";
...
var init_email_notifications = __esm({
  "api/auth/shared/email-notifications.ts"() {
    "use strict";
    init_functionsRoutes_0_7326988386381792();
    init_checked_fetch();
...
async function getMagicLinkByToken(db, token) {
  return await db.prepare(`
...
async function markMagicLinkAsUsed(db, token) {
  await db.prepare(`
    UPDATE magic_links 
...
async function getUserByEmail2(db, email) {
  return await db.prepare("SELECT * FROM users WHERE email = ?").bind(email).first();
}
__name(getUserByEmail2, "getUserByEmail2");
async function updatePreferredLoginMethod(db, email, method) {
  await db.prepare(`
    UPDATE users 
    SET preferred_login_method = ?, updated_at = datetime('now')
    WHERE email = ?
...
async function processPendingPayments3(db, email, userId, stripeSecretKey) {
  try {
    const pendingPayments = await db.prepare(`
      SELECT * FROM pending_payments 
      WHERE email = ? AND processed = 0
...
async function onRequestGet14(context) {
  try {
    ...
    try {
      ...
      const user = await getUserByEmail2(db, magicLink.email);
      if (!user) {
        return new Response(null, {
          status: 302,
      ...
      await processPendingPayments3(db, user.email, user.id, env.STRIPE_SECRET_KEY);
      const jwtSecret = env.JWT_SECRET;
      if (!jwtSecret) {
        return new Response(null, {
          status: 302,
...
async function createNetworkMonitor(db, data) {
  const result = await db.prepare(`
...
__name(createNetworkMonitor, "createNetworkMonitor");
async function getUserMonitors(db, userId) {
  const result = await db.prepare(`
...
__name(getUserMonitors, "getUserMonitors");
async function updateMonitor(db, id, userId, data) {
  const setClauses = [];
  const values = [];
...
__name(updateMonitor, "updateMonitor");
async function deleteMonitor(db, id, userId) {
  const result = await db.prepare(`
...
__name(deleteMonitor, "deleteMonitor");
async function monitorExists(db, userId, domain) {
  const result = await db.prepare(`
    SELECT EXISTS(
      SELECT 1 FROM network_monitors 
      WHERE user_id = ? AND domain = ?
    ) as exists
  `).bind(userId, domain).first();
  return result?.exists === 1;
}
__name(monitorExists, "monitorExists");
var init_db = __esm({
  "../src/lib/db.ts"() {
    "use strict";
    init_functionsRoutes_0_7326988386381792();
    init_checked_fetch();
    __name2(createNetworkMonitor, "createNetworkMonitor");
    __name2(getUserMonitors, "getUserMonitors");
    __name2(updateMonitor, "updateMonitor");
    __name2(deleteMonitor, "deleteMonitor");
    __name2(monitorExists, "monitorExists");
  }
});
...
async function onRequestGet15(context) {
  try {
    ...
    const monitors = await getUserMonitors(env.DB, userId);
    return new Response(JSON.stringify({
      success: true,
      data: monitors
    }), {
...
async function onRequestPost7(context) {
  try {
    ...
    const exists = await monitorExists(env.DB, body.user_id, body.domain);
    if (exists) {
      return new Response(JSON.stringify({
        success: false,
        error: "Monitor already exists for this domain"
      }), {
        status: 409,
        headers: { "Content-Type": "application/json" }
      });
    }
    const monitor = await createNetworkMonitor(env.DB, body);
    return new Response(JSON.stringify({
      success: true,
      data: monitor
    }), {
...
async function onRequestPut2(context) {
  try {
    ...
    const monitor = await updateMonitor(env.DB, body.id, body.user_id, body);
    if (!monitor) {
      return new Response(JSON.stringify({
        success: false,
        error: "Monitor not found or no changes made"
      }), {
        status: 404,
        headers: { "Content-Type": "application/json" }
      });
...
async function onRequestDelete(context) {
  try {
    ...
    const deleted = await deleteMonitor(env.DB, body.id, body.user_id);
    if (!deleted) {
      return new Response(JSON.stringify({
        success: false,
...
var routes;
var init_functionsRoutes_0_7326988386381792 = __esm({
  "../.wrangler/tmp/pages-OHMer6/functionsRoutes-0.7326988386381792.mjs"() {
    "use strict";
    init_callback();
    init_callback2();
    init_callback3();
...
// .wrangler/tmp/bundle-6TbaVW/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-6TbaVW/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.13542371417159882.js.map
