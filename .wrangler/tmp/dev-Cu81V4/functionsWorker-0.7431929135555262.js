...
// .wrangler/tmp/bundle-KygQc4/checked-fetch.js
...
// .wrangler/tmp/pages-fQ2eE7/functionsWorker-0.7431929135555262.mjs
...
var init_checked_fetch = __esm({
  "../.wrangler/tmp/bundle-2J1rJt/checked-fetch.js"() {
    "use strict";
...
var init_email_notifications = __esm({
  "api/auth/shared/email-notifications.ts"() {
    "use strict";
    init_functionsRoutes_0_8485085098830633();
    init_checked_fetch();
...
async function onRequestGet7(context) {
  try {
    ...
    const networks = ["awin", "cj", "impact", "rakuten", "shareasale", "tradedoubler", "webgains"];
    const processedDomains = await fetchNetworkHeartbeats(networks, uptimeKumaSecret);
...
async function fetchNetworkHeartbeats(networks, secret) {
  const domains = /* @__PURE__ */ new Map();
  for (const network of networks) {
    try {
      const response = await fetch(`http://152.42.135.243:3001/api/status-page/heartbeat/${network}?limit=10080`, {
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.heartbeatList && Object.keys(data.heartbeatList).length > 0) {
          const domain = processHeartbeatData(network, data);
          domains.set(network, domain);
        }
      } else {
        console.log(`No status page data for ${network}: ${response.status}`);
      }
    } catch (error) {
...
__name(fetchNetworkHeartbeats, "fetchNetworkHeartbeats");
function createTimeBucket(timestamp, hoursPerBucket) {
  const year = timestamp.getFullYear();
  const month = String(timestamp.getMonth() + 1).padStart(2, "0");
  const day = String(timestamp.getDate()).padStart(2, "0");
  if (hoursPerBucket < 1) {
    const totalMinutes = timestamp.getHours() * 60 + timestamp.getMinutes();
    const bucketMinutes = Math.floor(totalMinutes / (hoursPerBucket * 60)) * (hoursPerBucket * 60);
    ...
    const hour = Math.floor(timestamp.getHours() / hoursPerBucket) * hoursPerBucket;
    const hourStr = String(hour).padStart(2, "0");
    return `${year}-${month}-${day} ${hourStr}:00`;
  }
}
__name(createTimeBucket, "createTimeBucket");
function processHeartbeatData(network, data) {
  const heartbeats = [];
  Object.values(data.heartbeatList).forEach((monitorHeartbeats) => {
    if (Array.isArray(monitorHeartbeats)) {
      heartbeats.push(...monitorHeartbeats);
    }
  });
  const totalHeartbeats = heartbeats.length;
  const upHeartbeats = heartbeats.filter((h) => h.status === 1).length;
  const uptimePercentage = totalHeartbeats > 0 ? upHeartbeats / totalHeartbeats * 100 : 0;
  const validPings = heartbeats.filter((h) => h.ping && h.ping > 0).map((h) => h.ping);
  const avgResponseTime = validPings.length > 0 ? validPings.reduce((sum, ping) => sum + ping, 0) / validPings.length : 0;
  const timeBuckets = /* @__PURE__ */ new Map();
  heartbeats.forEach((heartbeat) => {
    const timestamp = new Date(heartbeat.time.replace(" ", "T"));
    const bucketKey = createTimeBucket(timestamp, 0.083);
    if (!timeBuckets.has(bucketKey)) {
      timeBuckets.set(bucketKey, { up: 0, total: 0, avgPing: 0 });
    }
    const bucket = timeBuckets.get(bucketKey);
    bucket.total++;
    if (heartbeat.status === 1) bucket.up++;
    if (heartbeat.ping && heartbeat.ping > 0) {
      bucket.avgPing = (bucket.avgPing * (bucket.total - 1) + heartbeat.ping) / bucket.total;
    }
  });
  const sortedBuckets = Array.from(timeBuckets.entries()).sort(([a], [b]) => a.localeCompare(b)).map(([bucketKey, data2]) => ({
    date: bucketKey,
    uptime: data2.total > 0 ? data2.up / data2.total : null
  }));
  const bucketUptime = sortedBuckets;
  return {
    domain: network,
...
var init_affiliate_network_uptime = __esm({
  "api/tools/affiliate-network-uptime.ts"() {
    ...
    __name2(fetchNetworkHeartbeats, "fetchNetworkHeartbeats");
    __name2(createTimeBucket, "createTimeBucket");
    __name2(processHeartbeatData, "processHeartbeatData");
  }
});
var routes;
var init_functionsRoutes_0_8485085098830633 = __esm({
  "../.wrangler/tmp/pages-fQ2eE7/functionsRoutes-0.8485085098830633.mjs"() {
    "use strict";
    init_callback();
    init_callback2();
    init_callback3();
...
// .wrangler/tmp/bundle-KygQc4/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-KygQc4/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7431929135555262.js.map
