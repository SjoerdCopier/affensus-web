...
// .wrangler/tmp/bundle-ZisxGF/checked-fetch.js
...
// .wrangler/tmp/pages-3f8feN/functionsWorker-0.11233783660013963.mjs
...
var init_checked_fetch = __esm({
  "../.wrangler/tmp/bundle-eRXokP/checked-fetch.js"() {
    "use strict";
...
var init_email_notifications = __esm({
  "api/auth/shared/email-notifications.ts"() {
    "use strict";
    init_functionsRoutes_0_27386561316137603();
    init_checked_fetch();
...
async function onRequestGet7(context) {
  try {
    ...
    let heartbeatData = {};
    try {
      const monitorsResponse = await fetch(`http://152.42.135.243:3001/api/monitor/list`, {
        headers: {
          "Authorization": `Bearer ${uptimeKumaSecret}`,
          "Accept": "application/json"
        }
      });
      if (monitorsResponse.ok) {
        const data = await monitorsResponse.json();
        monitorsData = data.monitorList || [];
        console.log(`Found ${monitorsData.length} monitors`);
      } else {
        console.error("Failed to fetch monitors:", monitorsResponse.status);
      }
      const sevenDaysAgo = /* @__PURE__ */ new Date();
      sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
      for (const monitor of monitorsData) {
        try {
          const heartbeatResponse = await fetch(`http://152.42.135.243:3001/api/monitor/beats/${monitor.id}?hours=168`, {
            headers: {
              "Authorization": `Bearer ${uptimeKumaSecret}`,
              "Accept": "application/json"
            }
          ...
          if (heartbeatResponse.ok) {
            const beats = await heartbeatResponse.json();
            heartbeatData[monitor.id] = beats.data || [];
            console.log(`Fetched ${heartbeatData[monitor.id].length} heartbeats for monitor ${monitor.name}`);
          }
        } catch (error) {
          console.error(`Error fetching heartbeats for monitor ${monitor.id}:`, error);
        }
      }
    } catch (error) {
      console.error("Error fetching monitor data:", error);
      try {
        const statusPageResponse = await fetch(`http://152.42.135.243:3001/api/status-page/heartbeat/affensus`, {
          headers: {
            "Accept": "application/json"
          }
        });
        if (statusPageResponse.ok) {
          ...
          heartbeatData = statusData.heartbeatList || {};
          console.log("Fetched heartbeat data from status page");
        }
      } catch (statusError) {
        console.error("Error fetching from status page:", statusError);
      }
    }
    if (Object.keys(heartbeatData).length === 0) {
      console.log("No heartbeat data available");
      return new Response(JSON.stringify({ error: "Unable to fetch heartbeat data from Uptime Kuma" }), {
        status: 503,
        headers: {
          "Content-Type": "application/json",
    ...
    const processedDomains = await processHeartbeatData(monitorsData, heartbeatData);
...
async function processHeartbeatData(monitors, heartbeatData) {
  const domains = /* @__PURE__ */ new Map();
  for (const monitor of monitors) {
    const domain = extractDomainFromMonitor(monitor.name);
    const type = extractTypeFromMonitor(monitor.name);
    ...
    const heartbeats = heartbeatData[monitor.id] || [];
    const dailyUptime = calculateDailyUptime(heartbeats);
    const avgResponseTime = heartbeats.length > 0 ? heartbeats.reduce((sum, hb) => sum + (hb.ping || 0), 0) / heartbeats.length : 0;
    const totalUpMinutes = dailyUptime.reduce((sum, day) => sum + (day.uptime || 0) * 1440, 0);
    const totalMinutes = dailyUptime.length * 1440;
    const uptimePercentage = totalMinutes > 0 ? totalUpMinutes / totalMinutes * 100 : 100;
    const existingUrlIndex = domainData.urls.findIndex((u) => u.type === type);
    if (existingUrlIndex >= 0) {
      domainData.urls[existingUrlIndex].avg_uptime_percentage = uptimePercentage;
      domainData.urls[existingUrlIndex].avg_response_time = avgResponseTime;
    } else {
    ...
    const existingUptimeIndex = domainData.day_uptime.findIndex((u) => u.type === type);
    if (existingUptimeIndex >= 0) {
      domainData.day_uptime[existingUptimeIndex].day_uptime = dailyUptime;
    } else {
      domainData.day_uptime.push({
        type,
        day_uptime: dailyUptime
      });
    }
  }
...
__name(processHeartbeatData, "processHeartbeatData");
function calculateDailyUptime(heartbeats) {
  const dailyData = /* @__PURE__ */ new Map();
  for (let i = 0; i < heartbeats.length - 1; i++) {
    const currentBeat = heartbeats[i];
    const nextBeat = heartbeats[i + 1];
    const currentTime = new Date(currentBeat.time);
    const nextTime = new Date(nextBeat.time);
    const date = currentTime.toISOString().split("T")[0];
    const durationMinutes = (nextTime.getTime() - currentTime.getTime()) / (1e3 * 60);
    if (!dailyData.has(date)) {
      dailyData.set(date, { totalMinutes: 0, upMinutes: 0 });
    }
    const dayData = dailyData.get(date);
    dayData.totalMinutes += durationMinutes;
    if (currentBeat.status === 1) {
      dayData.upMinutes += durationMinutes;
    }
  }
  const result = [];
  ...
  for (const date of sortedDates) {
    ...
    result.push({
      date,
      uptime: data.totalMinutes > 0 ? data.upMinutes / data.totalMinutes : null
    });
  }
  ...
  for (let i = 0; i < 7; i++) {
    ...
    if (!result.find((d) => d.date === dateStr)) {
      result.push({ date: dateStr, uptime: null });
    }
  }
  return result.sort((a, b) => a.date.localeCompare(b.date));
}
__name(calculateDailyUptime, "calculateDailyUptime");
...
function extractTypeFromMonitor(monitorName) {
  const parts = monitorName.split(" - ");
  if (parts.length > 1) {
    return parts[1];
  }
...
__name(extractTypeFromMonitor, "extractTypeFromMonitor");
var init_affiliate_network_uptime = __esm({
  "api/tools/affiliate-network-uptime.ts"() {
    ...
    __name2(processHeartbeatData, "processHeartbeatData");
    __name2(calculateDailyUptime, "calculateDailyUptime");
    __name2(extractDomainFromMonitor, "extractDomainFromMonitor");
    __name2(extractTypeFromMonitor, "extractTypeFromMonitor");
  }
});
var routes;
var init_functionsRoutes_0_27386561316137603 = __esm({
  "../.wrangler/tmp/pages-3f8feN/functionsRoutes-0.27386561316137603.mjs"() {
    "use strict";
    init_callback();
    init_callback2();
    init_callback3();
...
// .wrangler/tmp/bundle-ZisxGF/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ZisxGF/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.11233783660013963.js.map
